...
public class SessionTestController {
	...
	public String showSessionData(HttpServletRequest request, HttpSession session) {   
		Enumeration<String> attrs = session.getAttributeNames();   // request 객체의 getSession 메소드 호출로 가져오는 것도 가능   
	   ...
	   public String deleteAllSessionData(HttpSession session) {   
		   // invalidate 메소드 호출하여 세션 정보 모두 삭제 (해당 세션은 파기되며 이후 세션을 요 청하면 새로운 ID를 가진 새 새션이 생성됨)   
		   session.invalidate();    
